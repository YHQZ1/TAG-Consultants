...
function twentyseventeen_excerpt_more($link)
{
	...
	$link = sprintf(
		...
		sprintf(__(' "%s"</span>', 'twentyseventeen'))
	);
	return ' &hellip; ' . $link;
}
...
/**
 * 1. FORCE AUTHENTICATED SENDER HEADERS
 * This prevents the server from thinking the email is 'spoofed'
 */
add_action('phpmailer_init', function ($phpmailer) {
	$phpmailer->setFrom('info@tagconsultants.in', 'TAG Website');
});

/**
 * 2. CATCH HIDDEN ERRORS
 * If the email fails again, this will write the EXACT reason into 
 * your 'wp-content/debug.log' file on the server.
 */
add_action('wp_mail_failed', function ($wp_error) {
	error_log('MAIL ERROR: ' . print_r($wp_error->get_error_messages(), true));
});